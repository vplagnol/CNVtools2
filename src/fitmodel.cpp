...
vector<double> CNV_signal::GetPosterior() const 
{
  ...
  for (int i = 0; i != length; i++) {
    ...
    ret[ i + 6*length]  = nu[i];
    ret[ i + 7*length]  = proba_not_outlier[i];
  }
...
void CNV_signal::MaximizeAlpha(const int& t) 
{
  ...
  if(t==1){
    for (int i = 0; i != length; i++) {
      alphas[ cohort[i] - 1 ][ cn[i] ] += proba_not_outlier[ individual[i] ]*posterior[i];
    }
  ...
  if(t==2){
    for (int i = 0; i != length; i++) {
      alphas[0][ cn[i] ] += proba_not_outlier[ individual[i] ]*posterior[i];
    }
    double sum = 0.; 
...
{   
  ...
  if (failure == 1) {
    ...
    for(int i = 0; i != length; i++) {
      ...
      //if (temp[i] == 0) {cerr<<"i "<<temp[i]<<endl; exit(1);} //VP
    }
    this->FillGaps();
  }
...
void CNV_signal::MaximizeVariancesG()
{
  ...
  for (int i = 0; i != length; i++)  { 
    weights[i] = proba_not_outlier[ individual[i] ]*posterior[i];
...
void CNV_signal::MaximizeMeansG()
{
  ...
  for (int i = 0; i != length; i++)   {
    weights[i] = proba_not_outlier[ individual[i] ]*posterior[i]/variance[i];  //It perhaps makes sense that high var components get less reliable estimates of mean
  }
...
void CNV_signal::ComputePosterior ()
{
  ...
  for (int i = 0; i != nind; i++) {   //for each individual
	...
	//cout<<1./po <<"\t"<<posterior[ j*nind + i ]<<endl;
	//cout<<	posterior[ j*nind + i ] <<"\t"<<exp(logp[ j * nind + i ] -  max_logP[ i ] )<<endl;

	if (posterior[ j*nind + i ] < 0.0001)   {posterior[ j*nind + i ] = 0.;}
	if (1./po != 1./po) {
	  cerr<<posterior[ j*nind + i ]<<" makes no sense\n";
...
void CNV_signal::ExpectationG() 
{
  ...
  for (int i = 0 ; i != length; i++) {
    ...
    //if (p_disease == 0) {cout<<"kkkk "<<p_disease<<endl; exit(1);} //VP
    ...
    if (logp[i] != logp[i]) {
      ...
      throwmessage << "CNV_signal::Expectation : NaN in the likelihood computation : " 
		   << i << "\t" << variance[i] << "\t" << alpha[i] << "  " << log(alpha[i]) << "  " << cn[i] << "\t" << proba_disease[i]<<"\t"<<u<<endl;
...
void CNV_signal::Check_order()
{
  ...
  //cout<<"kkkkkkkkkkkk "<<nstrat_assoc<<endl;
